...
class DummyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"<h1>ZEBRA SMS Bot is Running!</h1>")
...
def get_admin_panel_text():
    ...
    text = (
        ...
        "💎 <i>ZEBRA SMS Bot • Live & Operating</i>"
...
async def monitor_loop(app):
    ...
    while True:
        ...
        try:
            ...
            r = await client_async.get(f"{BASE_URL}/publicapi/getupdate")
            ...
            # API error check
            if not isinstance(res, dict) or res.get("meta", {}).get("code", -1) != 0:
                error_msg = res.get("meta", {}).get("error") or res.get("message") or "Unknown API error"
                print(f"Monitor API Error: {error_msg}")
                await asyncio.sleep(CHECK_INTERVAL)
                continue
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    full_sms = row.get("message") or "No SMS Content"

                    ext_otp, ext_link = extract_link_and_otp(full_sms)
                    otp_code = ext_otp

                    # Unique key for deduplication
                    sms_key = f"{num}_{full_sms}"
...
async def fetch_number_async(range_str):
    ...
    try:
        ...
        url = f"{BASE_URL}/publicapi/getnum"

        r = await client_async.post(url, json={"range": range_str})
        ...
        try:
            ...
            print(f"fetch_number_async Error: Invalid JSON response")
            return None

        # API error check
        if not isinstance(data, dict) or data.get("meta", {}).get("code", -1) != 0:
            error_msg = data.get("meta", {}).get("error") or data.get("message") or "Unknown API error"
            print(f"fetch_number_async API Error: {error_msg}")
            return None

        rows = data.get("data", {}).get("rows", [])
        if not rows:
            print(f"fetch_number_async: No numbers available for range {range_str}")
            return None

        row = rows[0]
        number = row.get("number", "")
        if number:
            clean_num = normalize_number(str(number))
            return {
                "number": clean_num,
                "otp_now": False,
                "otp": None,
...
def main():
    ...
    # Start dummy HTTP server in background thread (for Render Web Service)
